  ...
  module Inventory
    ...
    module DB
      ...
      INDEX_DEFINITIONS = [
        ...
        { table: :groups_hosts, columns: %i[host_id group_id], unique: true, name: :idx_groups_hosts_host_id_group_id },
      ...
      def self.schema_migration_artifacts_missing?(version)
      ...
      def self.schema_indexes_missing?
      ...
      def self.dedupe_duplicate_rows!(table_name, columns, value_columns: [])
      ...
      def self.duplicate_keys(table_name, columns)
        @db[table_name]
          .select(*columns)
          .group(*columns)
          .having { count(id) > 1 }
      ...
      def self.reject_conflicting_duplicates!(table_name, key, rows, value_columns)
