...
2. Release environment protection rules were confirmed and documented on 2026-05-29 in `docs/release/release-environment-protection.md`. The GitHub `release` environment exists and is used by trusted publishing, but currently has no required deployment reviewers, wait timer, or deployment branch/tag policy configured.
