...
| RISK-SEC-005 | Open release-governance gap, not accepted | Medium | GitHub `release` environment exists for RubyGems trusted publishing, but has no required reviewers, wait timer, or deployment branch/tag policy configured as of 2026-05-29. | Release workflow only runs on `v*` tags, verifies tag/version alignment, runs full security-required check gate, and uses RubyGems trusted publishing/OIDC. | Decide whether to add environment protection rules before relying on GitHub environment approval as a release control. |
